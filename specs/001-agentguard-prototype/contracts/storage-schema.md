# Contract: SQLite Schema v1

Location `<DataDir>/agentguard.db`. Pragmas on open: `journal_mode=WAL`, `synchronous=NORMAL`, `foreign_keys=ON`, `busy_timeout=5000`. Migration `0001_initial` creates everything below inside one transaction and inserts `schema_version(1)`. The daemon refuses to start on a database whose max `schema_version` is greater than it knows.
...
Data that must survive restarts: everything above. No automatic deletion in v1.
